...
expect_equal(voxelSize(x), c(1, 1, 1))
expect_equal(worldTransform(x), diag(4))
...
expect_equal(length(voxelSize(series)), 3L)

## ...but the split can be stated explicitly, for a vector field say
field <- denseImage(array(0, c(4L, 4L, 4L, 3L)), spatial = 3L, voxelSize = c(2, 2, 2))
expect_equal(spatial(field), 3L)
slices <- denseImage(array(0, c(4L, 4L, 6L)), spatial = 2L, voxelSize = c(1, 1))
expect_equal(spatial(slices), 2L)
expect_equal(voxelSize(slices), c(1, 1))
...
expect_error(denseImage(array(0, c(2, 2, 2)), worldTransform = diag(3)), "4x4")
expect_error(denseImage(array(0, c(2, 2, 2)), worldTransform = rbind(diag(4)[1:3, ], c(1, 1, 1, 1))), "affine")
expect_error(denseImage(array(0, c(2, 2, 2)), voxelSize = c(1, NA, 1)), "must not be missing")

sheared <- diag(4)
sheared[1:3, 1:3] <- matrix(c(1, 0, 0, 0.3, 1, 0, 0, 0, 1), 3)
expect_error(denseImage(array(0, c(2, 2, 2)), worldTransform = sheared), "shear")

## --- Geometry accessors ----------------------------------------------------

las <- rbind(c(-2, 0, 0, 90), c(0, 2, 0, -126), c(0, 0, 2, -72), c(0, 0, 0, 1))
image <- denseImage(array(0, c(10L, 10L, 10L)), voxelSize = c(2, 2, 2), worldTransform = las)
expect_equal(worldTransform(image), las)
expect_equal(voxelSize(image), c(2, 2, 2))

## Replacing the voxel size must not touch rotation or translation -- this is
## the regression case for the original bug, where pixdim<- discarded both by
## resetting the whole transform to a diagonal matrix
...
expect_equal(voxelSize(replaced), voxelSize(x))
expect_equal(worldTransform(replaced), worldTransform(x))
...
## asDense() leaves an already-dense image alone but promotes a plain array
...
expect_equal(voxelSize(x * 2), voxelSize(x))
