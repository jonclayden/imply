...
#'
#' A NIfTI image's `xform` maps voxel to world coordinates, which is exactly
#' what [xform()] holds, so the conversion is a copy of the data plus a
...
fromNifti <- function (x, ...)
{
    ...
    voxelDims <- RNifti::pixdim(x)[seq_len(nSpatial)]
    units <- RNifti::pixunits(x)

    denseImage(values, pixdim = voxelDims, spatial = nSpatial,
               xform = RNifti::xform(x)[1:4, 1:4],
...
toNifti <- function (x, datatype = "auto")
{
    ...
    ## values the transform already implies would double them
    transform <- xform(x)
    voxelDims <- pixdim(x)
    header <- rep(1, 8)
...
fromMriImage <- function (x, ...)
{
    ...
    denseImage(values, pixdim = voxelDims, spatial = nSpatial, xform = transform, ...)
}
...
toMriImage <- function (x)
{
    ...
    tractor.base::asMriImage(as.array(x), voxelDims = pixdim(x), origin = originOf(x))
}
