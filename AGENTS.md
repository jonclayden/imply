...
R CMD check --as-cran --no-manual imply_0.1.0.tar.gz
```
...
- **The shell is fish.** Backslashes inside `Rscript -e '...'` get mangled (`\\b` becomes a backspace escape, not a regex word boundary). Write throwaway scripts to a file and run `Rscript file.R` instead. This has caused at least three phantom "bugs" that were only ever in the test harness.
